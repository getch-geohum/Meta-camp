import argparse
import os
from ssl_main_maml_unet import MeanTeacher
...
if __name__ == "__main__":
    ...
    for folder in os.listdir(args.root):
        ...
        for size in dsize:
            final_out_root = args.weight_path + '/' + '/mean_teacher_data_size/' + folder + '/{}'.format(str(size))
            print(final_out_root)
            if folder not in ['minawa_12feb2017', 'tza_nduta_21oct2016']:
                if not os.path.exists(final_out_root):
                    os.makedirs(final_out_root, exist_ok=True)
                adapter = MeanTeacher(root=data_path,
                    tr_size=size,
