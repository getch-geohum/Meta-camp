...
class GeneratEmbedFeatures:
    ...
    def generate_tsne_features(self):
        ...
        folds = sorted(os.listdir(self.data_root))
...
def argumentParser():
    ...
    parser.add_argument('--maml', help='Whether the model is trained with model agonstic approach', type=str, required=False)
...
if __name__ == "__main__":
    args = argumentParser()
    if args.mtype == 'seg':
        from main_maml_unet import DeepUnet
        model = DeepUnet()
        if args.maml is not None:
            model = l2l.algorithms.MAML(model, lr=0.001, first_order=True, allow_nograd=True)
        if args.weights is not None:
            model.load_state_dict(torch.load(args.weights))
        new_model = copy.deepcopy(model.model.encoder) 
        new_model.eval()
        generator = GeneratEmbedFeatures(data_root=args.data_root,
...
#         config = mmcv.Config.from_file(args.config)
